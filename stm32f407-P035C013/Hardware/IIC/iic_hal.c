...
/**
  ...
  * @param  found_addr Array to store found addresses (must be at least 128 elements)
  * @return Number of devices found
  * @note   This function will scan all possible 7-bit I2C addresses (1-127)
  *         and return a list of addresses that responded with ACK.
  *         Typical usage:
  *         uint8_t found_devices[128];
  *         uint8_t count = IIC_Scan(&bus, found_devices);
...
uint8_t IIC_Scan(iic_bus_t *bus, uint8_t found_addr[])
{
    ...
    // Scan all possible 7-bit I2C addresses (skip address 0 as it's reserved)
    for(addr = 1; addr < 128; addr++)
    {
        IICStart(bus);
        IICSendByte(bus, addr << 1);  // Try to write to the address
        
        if(IICWaitAck(bus) == 0)      // If device responds with ACK
        {
            found_addr[count] = addr; // Store the address
            count++;
        }
        
        IICStop(bus);
        delay_us(10);  // Small delay between address checks
        
        // Optional: Add a longer delay if bus is unstable
        // delay_ms(1);
    }
