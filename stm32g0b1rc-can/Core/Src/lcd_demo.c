...
//					case 2: LCD_ShowPicture(0, 0, 239, 171, gImage_img3); break;
            ...
            case STATE_HANDWRITING:
				...
				if (FT6236_Get_Touch_Count() > 0) {
					if(lastX != -1 && lastY != -1) {
						// 使用Bresenham算法画线
						LCD_DrawLine(lastX, lastY, FT6236_Instance.X_Pos, FT6236_Instance.Y_Pos, WHITE);
					}
                ...
                // 清屏按钮检测
                if (IsTouchInButton(FT6236_Instance.X_Pos, FT6236_Instance.Y_Pos)) {
                    LCD_FillRect_FastStatic(0, 0, SCREEN_WIDTH-1, SCREEN_HEIGHT-1, BLACK);
                    DrawClearButton();
                }
                break;
