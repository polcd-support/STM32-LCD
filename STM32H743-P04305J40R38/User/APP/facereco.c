...
uint16_t g_reg_time = 0;            /* 识别一个人脸所用时间 */
...
{
    frec_node *pnode;
    pnode = gui_memin_malloc(sizeof(frec_node));
...
void frec_node_free(frec_node * pnode,uint8_t mode)
{
    ...
    if (pnode->pname) gui_memin_free(pnode->pname);     /* 释放pname内存 */
    
    if (pnode->pnum) gui_memin_free(pnode->pnum);       /* 释放pnum内存 */
    
    if (pnode->sex) gui_memin_free(pnode->sex);         /* 释放sex内存 */
...
uint8_t appplay_frec_show_result(frec_node *pnode,_frec_obj *frecdev,uint16_t time)
{
    ...
    {
        fontsize = 24;
        cyoff = 40 + 80 + 500 + 70,cwidth = 600;cheight = 380;
        cixoff = 50,ciyoff = 60;
...
uint8_t appplay_frec_add_a_face(frec_node *pnode,_frec_obj *frecdev,uint8_t mode)
{
    ...
    {
        fontsize = 24;
        cyoff = 130+32,cwidth = 600;cheight = 380;
        cixoff = 50,
        btn_height = 60; 
        t9height = 488; 
    }
    ...
    if (mode == 1)  /* 添加新人脸,需要申请内存 */
    {
        ...
        pnode->sex = gui_memin_malloc(20);	/* 申请20字节内存,用于存储性别信息 */
...
{ 
    ...
    if (lcddev.id == 0X1963 || lcddev.id == 0X5510 || lcddev.id == 0X7084 || lcddev.id == 0X9806)
    {   
        frecdev->height = 640; 
    }
...
{ 
    ...
    { 
        ...
        if (head->pnum)
        {
            ...
            dcmi_start();                   /* 启动传输  */
            tim6_int_init(100 - 1,20000 - 1);   /* 10Khz计数频率,加100是10ms,用于统计人脸识别时间 */
            system_task_return = 0; 
            lcd_clear(BLACK);
                        ...
                        case KEY0_PRES:         /* 识别人脸 */
                            ...
                            {  
                                ...
                                if(ptemp)
                                {
                                    ...
                                    appplay_frec_show_result(ptemp,&frec_dev,g_reg_time*10);    /* 显示识别结果 */
                                }    
                            }
