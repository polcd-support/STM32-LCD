...
extern volatile uint8_t g_framecnt;     /* 统一的帧计数器 */
extern volatile uint8_t g_framecntout;  /* 统一的帧计数器输出变量 */
 ...
 * @retval      无
 */
void video_audio_dma_callback(void)
{
    videodev.saiplaybuf++;
    ...
    {
        DMA2_Stream3->M0AR = (uint32_t)videodev.saibuf[videodev.saiplaybuf]; /* 指向下一个buf */
    }
    else
    {
        DMA2_Stream3->M1AR = (uint32_t)videodev.saibuf[videodev.saiplaybuf]; /* 指向下一个buf */
    }
}
...
void video_info_upd(__videodev *videodevx, _progressbar_obj *videoprgbx, _progressbar_obj *volprgbx, AVI_INFO *aviinfo, uint8_t flag)
{
    ...
    if (oldsec != cursec)
    {
        ...
        sprintf((char *)buf, "%02dfps", g_framecntout);
...
uint8_t video_play_mjpeg(void)
{
    ...
    while (res == 0)
    {
        ...
        if (res == 0)
        {
            ...
            tim6_int_init(g_avix.SecPerFrame / 100 - 1, 20000 - 1);  /* 10Khz计数频率,加1是100us,用于视频帧间隔控制 */
            tim8_int_init(10000 - 1, 20000 - 1);         /* 10Khz计数,1秒钟中断一次,用于帧计数 */
            ...
            if (g_avix.SampleRate)    /* 有音频信息,才初始化 */
            {
                ...
                sai1_samplerate_set(g_avix.SampleRate);/* 设置采样率 */
                sai1_tx_dma_init(videodev.saibuf[1], videodev.saibuf[2], g_avix.AudioBufSize / 2,1);   /* 配置DMA */
                
                sai1_tx_callback = video_audio_dma_callback; /* 回调函数指向sai_DMA_Callback */
                
                videodev.saiplaybuf = 0;
                saisavebuf = 0;
                sai1_play_start();   /* 开启sai播放 */
            }
            ...
            while (playflag == 0)   /* 播放循环 */
            {
                if (videodev.status & (1 << 0) && videodev.status & (1 << 1))
                {
                    if (g_avix.StreamID == AVI_VIDS_FLAG)   /* 视频流 */
                    {
                        ...
                        while (g_avi_frameup == 0); /* 等待时间到达(在TIM6的中断里面设置为1) */
                ...
                if (dirmode == 0)   /* 竖屏模式,才检测触摸屏 */
                {
                    ...
                    if (res)   /* 被按下了,并且松开了,执行快进快退 */
                    {
                        ...
                        if (videodev.status & 1 << 0)sai1_play_stop(); /* 非暂停时,开始快进快退,关闭音频 */
                    ...
                    {
                        if (videodev.status & 1 << 0)sai1_play_start(); /* 非暂停状态,快进快退结束,打开音频 */
