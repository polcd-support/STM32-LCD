...
extern uint8_t mem2base[MEM2_MAX_SIZE];	
...
void exeplay_app_check(void)
{
    if (rtc_read_bkr(1) == 0X5050)                                                          /* 检查BKP1,如果为0X5050,则说明需要执行app代码 */
    {
        ...
        my_mem_copy((uint8_t*)EXEPLAY_APP_BASE,(uint8_t*)EXEPLAY_SRC_BASE,EXEPLAY_APP_SIZE);/* 拷贝EXEPLAY_APP_SIZE字节 */
        ...
        SysTick->CTRL = 0;                                                                  /* 关闭SYSTICK中断,以免影响APP执行 */
        jump2app();                                                                         /* 执行app代码 */
    }
}
...
uint8_t exe_play(void)
{
    ...
    while (rval == 0)
    {
        ...
        if (flistbox->dbclick == 0X81)      /* 双击文件了 */
        {
            ...
            if (pname == NULL) rval = 1;    /* 申请失败 */
            {
                ...
                if (f_exe->obj.objsize < MEM2_MAX_SIZE)             /* 可以放的下,并且用户确定执行 */
                {  
                    gui_memset(mem2base,0,f_exe->obj.objsize);      /* 清空这片内存 */
                    p = mem2base;
                    readlen = 0;
                    
                    while (readlen < f_exe->obj.objsize)            /* 循环方式,读出BIN的所有内容 */
                    {
                        rval = f_read(f_exe,buf,1024,(UINT*)&bread);    /* 读出txt里面的内容 */
                        readlen += bread;
                    ...
                    rval = f_read(f_exe,mem2base,f_exe->obj.objsize,(UINT*)&bread1);/* 读出BIN的所有内容 */
                    
                    if (rval) break;                                                /* 打开失败,直接退出 */
                    if (((*(volatile uint32_t*)(mem2base+4))&0xFF000000) == 0x24000000) /* 判断是否为0X20XXXXXX.检测APP的合法性 */
                    {
    ...
    gui_memin_free(pname);          /* 释放内存 */
    gui_memin_free(exeplayinfo);    /* 释放内存 */
