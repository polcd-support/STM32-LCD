...
{
    uint32_t *pbuf;
    uint32_t *qr_plcd = g_ltdc_framebuf[lcdltdc.activelayer];   /* 指向显存地址减少数组访问,提高速度 */
    uint16_t *disbuf;
    uint16_t i;
    uint32_t temp;
...
void qr_decode_play(void)
{
    ...
    delay_ms(500);      /* OV5640初始化不完全，会导致闪退 */
    ...
    { 
        ...
        if (lcdltdc.pwidth != 0) ov5640_write_reg(0X3821,0x00); /* 不镜像 */
...
void qr_encode_play(void)
{
    ...
    app_filebrower((uint8_t*)APP_MFUNS_CAPTION_TBL[16][gui_phy.language],0X05); /* 显示标题 */
    g_point_color = WHITE;  
