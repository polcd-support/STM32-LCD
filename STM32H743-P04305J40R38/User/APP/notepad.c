...
void notepad_new_pathname(uint8_t *pname)
{
    ...
    else if (gui_phy.memdevflag & (1 << 3))sprintf((char *)pname, "3:TEXT/TEXT%04d%02d%02d%02d%02d%02d.txt", calendar.year, calendar.month, calendar.date, calendar.hour, calendar.min, calendar.sec);    /* SD卡不存在,则保存在U盘 */
}
