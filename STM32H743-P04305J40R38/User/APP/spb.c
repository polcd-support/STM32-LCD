...
uint8_t spb_init(uint8_t mode)
{
    ...
    else if(lcddev.width == 800)     /* 对于800*1280的LCD屏幕 */
    { 
        icowidth = 110; 
        micoyoff = 18;
        lcdtype = 5;
    ...
    g_sramlcdbuf = gui_memex_malloc((uint32_t)spbdev.spbheight * spbdev.spbwidth * 7);/* 申请3.5帧缓存大小 */
    ...
    res = piclib_ai_load_picfile(spb_bkpic_path_tbl[lcdtype],0,0,spbdev.spbwidth*SPB_PAGE_NUM + spbdev.spbahwidth * 2,spbdev.spbheight,0);/* 加载背景图片 */
