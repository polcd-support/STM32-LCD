...
uint8_t*const lwip_webserver_path_tbl[11]=
{
...
uint8_t *const APP_SYSTEM_APPPATH = "/SYSTEM/APP";      /* APP文件夹路径径 */
uint8_t *const APP_SYSTEM_SPBPATH = "/SYSTEM/SPB";      /* SPB文件夹路径 */
uint8_t *const APP_SYSTEM_LWIPPATH = "/SYSTEM/LWIP";    /* LWIP文件夹路径 */
...
uint8_t*const APP_NAND_TESTFILE_SRCPATH="/NAND测试文件";     //NADN 测试文件源路径
uint8_t*const APP_NAND_TESTFILE_DSTPATH="2:";                //NAND 测试文件目标路径
...
uint8_t app_system_update(uint8_t(*fcpymsg)(uint8_t *pname, uint8_t pct, uint8_t mode), uint8_t *src)
{
    ...
    totsize += exfuns_get_folder_size(p_app);   /* 得到4个文件夹的总大小 */
    totsize += exfuns_get_folder_size(p_spb);
    totsize += exfuns_get_folder_size(p_lwip);
    totsize += exfuns_get_folder_size(p_test);
    totsize += exfuns_get_folder_size(p_nand_test);
    printf("totsize:%d\r\n", totsize);
    ...
    else        /* 第一阶段OK,启动第二阶段 */
    {
        ...
        {
            res = exfuns_folder_copy(fcpymsg, p_lwip, (uint8_t *)APP_SYSTEM_DSTPATH, &totsize, &cpdsize, 1);
            ...
            else
            {
                ...
                {
                    res = exfuns_folder_copy(fcpymsg,p_nand_test,(uint8_t*)APP_NAND_TESTFILE_DSTPATH,&totsize,&cpdsize,1);
                    if (res)
                    {
...
{
    *sn0 = *(volatile uint32_t *)(0x1FF0F420);
    *sn1 = *(volatile uint32_t *)(0x1FF0F424);
    *sn2 = *(volatile uint32_t *)(0x1FF0F428);
}
...
uint8_t app_system_parameter_init(void)
{
    ...
    es8388_read_para(&es8388set);   /* 读取ES8388设置数据 */
    ...
    if (es8388set.saveflag != 0X0A) /* 之前没有保存过 */
    {
        es8388set.mvol = 20;        /* 音量:20 */
        for(i = 0;i < 5;i ++) es8388set.cfreq[i] = 0;   /* 中心频率都设置为代号0*/
        es8388set.d3 = 0;           /* 3d效果关闭 */
        for(i = 0;i < 5;i ++) es8388set.freqval[i] = 12;/* 全部设置位0dB增益 */
        es8388set.speakersw = 1;    /* 喇叭开启 */
        es8388set.saveflag = 0X0A;  /* 设置保存标记 */
        es8388_save_para(&es8388set);   /* 保存ES8388的设置信息 */ 
    }
    ...
    app_es8388_setall();                        /* 设置WM8978参数 */
    app_es8388_volset(0);                       /* 暂时关闭输出 */
    tim3_ch2_pwm_init(100, 20000 - 1);           /* 初始化TIM3_CH2 PWM输出,作为LCD的背光控制 */
...
void app_lcd_auto_bklight(void)
{
    ...
    {
        if(lcddev.id == 0X1963) lcd_ssd_backlight_set(0);
    ...
    else
    {
        als /= 20;      /* 缩小20倍 */
        if(als > 70)als = 70;
        if(lcddev.id == 0X1963)lcd_ssd_backlight_set(30 + als);
 ...
 * @retval      无
 */
void app_es8388_volset(uint8_t vol)
{
    if (vol > 30)vol = 30;

    if (es8388set.speakersw)es8388_spkvol_set(vol); /* 开启喇叭 */
    else es8388_spkvol_set(0);                      /* 关闭喇叭 */
...
/**
 * @brief       es8388 EQ设置
 * @param       eqx:频段编号,0~4,对应EQ1~5
 * @retval      无
 */
void app_es8388_eqset(_es8388_obj *wmset,uint8_t eqx)
{
    switch(eqx)
    {
        case 0: /* EQ1设置 */
            es8388_eq1_eet(wmset->cfreq[eqx],wmset->freqval[eqx]);
            break;
        case 1:
            es8388_eq2_eet(wmset->cfreq[eqx],wmset->freqval[eqx]);
            break;
        case 2:
            es8388_eq3_eet(wmset->cfreq[eqx],wmset->freqval[eqx]);
            break;
        case 3:
            es8388_eq4_eet(wmset->cfreq[eqx],wmset->freqval[eqx]);
            break;
        case 4:
            es8388_eq5_eet(wmset->cfreq[eqx],wmset->freqval[eqx]);
            break;
    }
}

/**
 * @brief       设置所有es8388参数(音频输出部分)
 * @param       无
 * @retval      无
 */
void app_es8388_setall(void)
{
    uint8_t i = 0;
