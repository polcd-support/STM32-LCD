...
/* 字库区域占用的总扇区数大小(4个字库+unigbk表+字库信息=6302984字节,约占1539个25QXX扇区,一个扇区4K字节) */
 ...
 * 从第25MB地址开始存放字库
 * 前面25MB大小被文件系统占用
 * 25MB后紧跟4个字库+UNIGBK.BIN,总大小6.01M, 1539 个扇区,被字库占用了,不能动!
 * 31.01M以后, 用户可以自由使用. 建议用最后的100K字节比较好
 */
#define FONTINFOADDR        25 * 1024 * 1024
...
uint8_t fonts_update_font(uint16_t x, uint16_t y, uint8_t size, uint8_t *src, uint16_t color)
{
    ...
    for (i = 0; i < 5; i++) /* 先查找文件UNIGBK,GBK12,GBK16,GBK24,GBK32 是否正常 */
    {
