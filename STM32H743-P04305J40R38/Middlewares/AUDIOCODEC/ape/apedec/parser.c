...
int ape_parseheader(FIL* fx, struct ape_ctx_t* ape_ctx)
{
    ...
    f_read(fx,&ape_ctx->magic,4,(uint32_t*)&n);
    if (n != 4) return -1;
    ...
    if (ape_ctx->fileversion >= 3980)
    {
        ...
        if (read_uint32(fx,&ape_ctx->wavtaillength) < 0)
            return -1;
		f_read(fx,&ape_ctx->md5,16,(uint32_t*)&n);
        if ( n!= 16)
            return -1; 
    ...
    {
        ape_ctx->seektable = (uint32_t *)mymalloc(SRAMIN,ape_ctx->seektablelength);
        ...
        for (i=0; i < ape_ctx->seektablelength / sizeof(uint32_t); i++)
        {
            ...
            {
                 myfree(SRAMIN,ape_ctx->seektable);
                 return -1;
            }
        }
    }
