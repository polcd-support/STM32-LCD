...
#include "./MJPEG/JPEG/jinclude.h"
#include "./MJPEG/JPEG/jpeglib.h"
...
/* 在mjpeg.c里面定义 */
extern uint16_t g_imgoffx, g_imgoffy; /* 图像在x,y方向的偏移量 */
extern uint16_t g_outlinecnt;        /* 输出行计数器 */
extern uint16_t *p_outlinebuf;       /* 输出行缓存,≥图像宽度*2字节 */
 ...
 * @retval      无
 */
void ltdc_color_fill2(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
                      ...
                      JSAMPARRAY output_buf)
{
    ...
    if (lcdltdc.width != 0) //RGB屏
    {
        for (col = cinfo->output_width >> 1; col > 0; col--)
        {
            ...
            p_outlinebuf[(col << 1) - 1] = (range_limit[y + cblue] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cred] >> 3;
            ...
            //outptr[RGB_BLUE] =  range_limit[y + cblue];
            //outptr += RGB_PIXELSIZE;
            p_outlinebuf[(col << 1) - 2] = (range_limit[y + cblue] >> 3) << 11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cred] >> 3;

        }

        ltdc_color_fill2(g_imgoffx, g_outlinecnt, cinfo->output_width + g_imgoffx - 1, g_outlinecnt, p_outlinebuf); //DM2D填充
        g_outlinecnt++;
                      ...
                      JSAMPARRAY output_buf)
{
    ...
    if (lcdltdc.width != 0) //RGB屏
    {
        ...
        for (col = cinfo->output_width >> 1; col > 0; col--)
        {
            ...
            p_outlinebuf[(col << 1) - 1] = range_limit[y + cblue] >> 3 | (range_limit[y + cgreen] >> 2) << 5 | (range_limit[y + cred] >> 3) << 11;

            y  = GETJSAMPLE(*inptr00++);
            //pixel.color.B = range_limit[y + cblue] >> 3;
            //pixel.color.G = range_limit[y + cgreen] >> 2;
            //pixel.color.R = range_limit[y + cred] >> 3;
            p_outlinebuf[(col << 1) - 2] = range_limit[y + cblue] >> 3 | (range_limit[y + cgreen] >> 2) << 5 | (range_limit[y + cred] >> 3) << 11;
        }
