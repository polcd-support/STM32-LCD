...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  //如果满足bInterfaceClass=3，bInterfaceSubClass=0，则执行后续初始化（针对USB游戏手柄）
  if(phost->device.CfgDesc.Itf_Desc[0].bInterfaceClass == 3 && phost->device.CfgDesc.Itf_Desc[0].bInterfaceSubClass == 0)
  {
    interface = 0;  
  }
  ...
  else if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceSubClass  == 0)    //游戏手柄
  {
...
HID_TypeTypeDef USBH_HID_GetDeviceType(USBH_HandleTypeDef *phost)
{
  ...
  if (phost->gState == HOST_CLASS)
  {
    ...
    else
    {
      ...
      else if (InterfaceProtocol == 0)    //添加USB游戏手柄支持
     {
       type = HID_GAMEPAD;
     }
    }
