...
/**
 * @brief       交换两个u16的值
 * @param       p1              : 参数1,交换后成参数2
 * @param       p2              : 参数2,交换后成参数1
 * @retval      无
 */
...
void gui_fill_triangle(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    ...
    for (int y = y1; y <= y2; y++)
    {
        ...
        alpha = (float)(y - y0) / total_height;
        beta = (float)(y - y1) / segment_height;
...
{
    void *temp;
    temp = (void *)mymalloc(SRAMEX, size);
    return temp;
}
