...
uint8_t lwip_comm_mem_malloc(void)
{
    ...
    my_mem_set(memp_memory,0,mempsize);
    my_mem_set(ram_heap,0,ramheapsize);
    my_mem_set(TCPIP_THREAD_TASK_STK,0,TCPIP_THREAD_STACKSIZE * 4);
...
{
    uint32_t sn0;
    sn0 = *(volatile uint32_t *)(0x1FF1E800); /* 获取STM32的唯一ID的前24位作为MAC地址后三字节 */
...
extern volatile uint8_t system_lwip_sta; /* lwip任务运行标志 */

/**
 * @brief       LWIP初始化(LWIP启动的时候使用)
 * @param       无
 * @retval      0,成功; 1,内存错误; 2,LAN8720初始化失败; 3,网卡添加失败;
 */
uint8_t lwip_comm_init(void)
{
    ...
    if (ethernet_init()) return 2;       /* 初始化PHY失败 */
    
    tcpip_init(NULL, NULL);             /* 初始化tcp ip内核,该函数里面会创建tcpip_thread内核任务 */
    
    system_lwip_sta = 1;                /* 标记LWIP任务运行中!! */
#if LWIP_DHCP       /* 使用动态IP */
    ipaddr.addr = 0;
    netmask.addr = 0;
    gw.addr = 0;
...
void lwip_comm_destroy(void)
{
...
#endif
    RCC->AHB1RSTR |= 1 << 15;       /* 复位以太网 */
    delay_ms(100);
    RCC->AHB1RSTR &= ~(1 << 15);    /* 复位结束 */
    ...
    memset(&g_eth_handler,0,sizeof(ETH_HandleTypeDef)); /* LAN8720_ETHHandle结构体数据清零 */
    pcf8574_write_bit(ETH_RESET_IO,0);      /* 硬件复位LAN8720A */
    sys_gpio_af_set(GPIOB,1<<11,7);            /* PB11,AF7,恢复为串口3 RX功能 */
    system_lwip_sta = 0;                    /* lwip任务运行结束 */
}
