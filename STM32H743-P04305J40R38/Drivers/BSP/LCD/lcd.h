...
#define LCD_WR_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */

#define LCD_RD_GPIO_PORT                GPIOD
#define LCD_RD_GPIO_PIN                 SYS_GPIO_PIN4
#define LCD_RD_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */

#define LCD_BL_GPIO_PORT                GPIOB
#define LCD_BL_GPIO_PIN                 SYS_GPIO_PIN5
#define LCD_BL_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 1; }while(0)   /* 所在IO口时钟使能 */

/* LCD_CS(需要根据LCD_FMC_NEX设置正确的IO口) 和 LCD_RS(需要根据LCD_FMC_AX设置正确的IO口) 引脚 定义 */
#define LCD_CS_GPIO_PORT                GPIOD
#define LCD_CS_GPIO_PIN                 SYS_GPIO_PIN7
#define LCD_CS_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */

#define LCD_RS_GPIO_PORT                GPIOD
#define LCD_RS_GPIO_PIN                 SYS_GPIO_PIN13
#define LCD_RS_GPIO_CLK_ENABLE()        do{ RCC->AHB4ENR |= 1 << 3; }while(0)   /* 所在IO口时钟使能 */
...
#define LCD_FMC_BCRX        FMC_Bank1_R->BTCR[(LCD_FMC_NEX - 1) * 2]        /* BCR寄存器,根据LCD_FMC_NEX自动计算 */
#define LCD_FMC_BTRX        FMC_Bank1_R->BTCR[(LCD_FMC_NEX - 1) * 2 + 1]    /* BTR寄存器,根据LCD_FMC_NEX自动计算 */
#define LCD_FMC_BWTRX       FMC_Bank1E_R->BWTR[(LCD_FMC_NEX - 1) * 2]       /* BWTR寄存器,根据LCD_FMC_NEX自动计算 */
 ...
 * 开发板使用FMC_NE4连接LCD_CS, FMC_A6连接LCD_RS ,16位数据线,计算方法如下:
 * 首先FMC_NE4的基地址为: 0X6C00 0000;     NEx的基址为(x=1/2/3/4): 0X6000 0000 + (0X400 0000 * (x - 1))
 * FMC_A6对应地址值: 2^6 * 2 = 0X80;       FMC_Ay对应的地址为(y = 0 ~ 25): 2^y * 2
 *
