...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
    ...
    if (lcddev.id == 0X7796) return r;  /* 7796 一次读取一个像素值 */
...
void lcd_set_window(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
    ...
    else    /* 9341/5310/7789/1963/7796/9806横屏等 设置窗口 */
    {
        lcd_wr_regno(lcddev.setxcmd);
        lcd_wr_data(sx >> 8);
        lcd_wr_data(sx & 0XFF);
...
void lcd_init(void)
{
    ...
    else                            /* 是MCU屏幕 */
    {
        ...
        LCD_FMC_BTRX |= 78 << 8;    /* 数据保存时间(DATAST)为80个fmc_ker_ck = 4.5ns * 78 = 351ns */

        /* 写时序控制寄存器 */
        LCD_FMC_BWTRX |= 0 << 28;   /* 模式A */
        LCD_FMC_BWTRX |= 15 << 0;   /* 地址建立时间(ADDSET)为12个fmc_ker_ck = 4.5ns * 15 = 67.5n */

        /* 某些液晶驱动IC的写信号脉宽，最少也得50ns。 */
        LCD_FMC_BWTRX |= 15 << 8;   /* 数据保存时间(DATAST)为12个fmc_ker_ck = 4.5ns * 15 = 67.5n */

        /* 使能BANK,区域x */
        LCD_FMC_BCRX |= 1 << 0;     /* 使能BANK，区域x */
        LCD_FMC_BCRX |= (uint32_t)1 << 31;  /* 使能FMC */

        lcd_opt_delay(0XFFFFF);     /* 初始化FMC后,必须等待一定时间才能开始初始化 */
            ...
            if (lcddev.id != 0x7789)        /* 也不是ST7789, 尝试是不是 NT35310 */
            {
                ...
                if (lcddev.id != 0X5310)    /* 也不是NT35310,尝试看看是不是ST7796 */
                {
                    ...
                    if (lcddev.id != 0x7796)    /* 也不是ST7796,尝试看看是不是NT35510 */
                    {
                        ...
                        delay_ms(5);                    /* 等待5ms, 因为0XC501指令对1963来说就是软件复位指令,等待5ms让1963复位OK再操作 */
    ...
    /* 初始化完成以后,提速 */
    if (lcddev.id == 0X7789 || lcddev.id == 0X9341 )  /* 7789/9341/1963 提速 */
    {
        /* 重新配置写时序控制寄存器的时序 */
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 5 << 0;        /* 地址建立时间(ADDSET)为5个fmc_ker_ck = 22.5 ns */
        LCD_FMC_BWTRX |= 5 << 8;        /* 数据保存时间(DATAST)为5个fmc_ker_ck = 22.5 ns */
    }
    if (lcddev.id == 0X1963) /* 如果是这几个IC, 则设置WR时序为最快 */
    {
        /* 重新配置写时序控制寄存器的时序 */
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 7 << 0;        /* 地址建立时间(ADDSET)为7个fmc_ker_ck = 31.5 ns */
        LCD_FMC_BWTRX |= 7 << 8;        /* 数据保存时间(DATAST)为7个fmc_ker_ck = 31.5 ns */
    }
    if (lcddev.id == 0X5510 || lcddev.id == 0X5310 || lcddev.id == 0X7796 || lcddev.id == 0X9806) /* 如果是这几个IC, 则设置WR时序为最快 */
    {
        /* 重新配置写时序控制寄存器的时序 */
        LCD_FMC_BWTRX &= ~(0XF << 0);   /* 地址建立时间(ADDSET)清零 */
        LCD_FMC_BWTRX &= ~(0XF << 8);   /* 数据保存时间清零 */
        LCD_FMC_BWTRX |= 3 << 0;        /* 地址建立时间(ADDSET)为3个fmc_ker_ck = 13.5 ns */
        LCD_FMC_BWTRX |= 3 << 8;        /* 数据保存时间(DATAST)为3个fmc_ker_ck = 13.5 ns */
    }
