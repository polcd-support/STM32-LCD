...
void nrf24l01_init(void)
{
    ...
    /* 对 PB14(F_CS) 进行设置, 避免SPI FLASH 影响NRF24L01操作 */
    sys_gpio_set(GPIOB, SYS_GPIO_PIN14,
             SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* PB14 推挽输出 */
    sys_gpio_pin_set(GPIOB, SYS_GPIO_PIN14, 1); /* PB14 输出1 防止25QXX干扰NRF24L01使用 */

    spi2_init();                /* 初始化SPI */

    /* 针对NRF的特点修改SPI的设置 */
    SPI2->CR1 &= ~(1 << 0);     /* SPE=0,SPI设备失能 */
    SPI2->CFG2 &= ~(1 << 25);   /* CPOL=0,空闲状态下,SCK为低电平 */
    SPI2->CFG2 &= ~(1 << 24);   /* CPHA=0,数据采样从第1个时间边沿开始 */
...
uint8_t nrf24l01_check(void)
{
    ...
    uint8_t i;
    spi2_set_speed(SPI_SPEED_32);                           /* spi速度为6.25Mhz（24L01的最大SPI时钟为10Mhz） */
