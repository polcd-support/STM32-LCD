...
void LCD_FillRect_FastStatic(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) {
    ...
    while (remaining > 0) {
        ...
        HAL_SPI_Transmit_DMA(&hspi1, buffer, currentPixels * 2);
...
void DrawColorBars(void) {
    const uint16_t colors[COLOR_BAR_NUM] = {RED, GREEN, BLUE, YELLOW, CYAN, MAGENTA};
    for (uint8_t i=0; i<COLOR_BAR_NUM; i++) {
        LCD_FillRect_FastStatic(i*BAR_WIDTH, 0, ((i+1)*BAR_WIDTH)-1, SCREEN_HEIGHT-1, colors[i]);
    }
}
