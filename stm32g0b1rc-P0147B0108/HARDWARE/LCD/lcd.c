...
void DrawColorBars(void) {
    ...
    for (uint8_t i = 0; i < COLOR_BAR_NUM; i++) {
        uint16_t start_x = (uint16_t)((uint32_t)i * SCREEN_WIDTH / COLOR_BAR_NUM);
        uint16_t end_x = (uint16_t)(((uint32_t)(i + 1) * SCREEN_WIDTH / COLOR_BAR_NUM) - 1);

        LCD_FillRect_FastStatic(start_x, 0, end_x, SCREEN_HEIGHT - 1, colors[i]);
    }
}
